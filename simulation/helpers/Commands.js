...
var g_Commands = {
	"hero-choice": function(player, cmd, data)
	{
		if (typeof TriggerHelper === "undefined")
			return;
		...
		let entities = TriggerHelper.GetPlayerEntitiesByClass(player, ["CivCentre"]);
		if (!entities || !entities.length)
			entities = TriggerHelper.GetPlayerEntitiesByClass(player, ["Unit"]);
		if (!entities || !entities.length)
			entities = TriggerHelper.GetPlayerEntitiesByClass(player, ["Structure"]);
		let spawnPoint;
	...
	"rotate-to-point": function(player, cmd, data)
	{
		GetFormationUnitAIs(data.entities, player).forEach(cmpUnitAI => {
			cmpUnitAI.Rotate(cmd.point, cmd.queued);
		});
	},
	...
	"delete-entities": function(player, cmd, data)
	{
		for (let ent of data.entities)
		{
			if (!data.controlAllUnits)
			{
				...
				if (cmpGarrisonHolder && cmpGarrisonHolder.GetAttackerEntities().length != 0)
					continue;
			}
...
"regroup": function(player, cmd, data)
	...
	"disable-range": function(player, cmd, data)
	{
		for (let ent of data.entities)
		{
			...
			if (cmd.disable)
				cmpAttack.SetNoRange();
			else
				cmpAttack.SetCanRange();
		}
	},
...
function GetMergedFormationUnitAIs(ents, player, formationTemplate)
{
	...
	if (ents.length == 1)
	{
		...
		if (!cmpUnitAI)
			return [];
/*
		let cmpFormationControler = Engine.QueryInterface(cmpUnitAI.GetFormationController(), IID_Formation);
		ents = cmpFormationControler.GetMembers();
*/
