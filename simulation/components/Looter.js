...
{
	let cmpLoot = Engine.QueryInterface(targetEntity, IID_Loot);
	if (!cmpLoot)
		return;

	// Collect resources carried by workers and traders
	let cmpResourceGatherer = Engine.QueryInterface(targetEntity, IID_ResourceGatherer);
	let cmpTrader = Engine.QueryInterface(targetEntity, IID_Trader);
