...
Player.prototype.GetFormationEntities = function()
{
...
Player.prototype.OnGlobalOwnershipChanged = function(msg)
{
	...
	let cmpFormation = Engine.QueryInterface(msg.entity, IID_Formation);
	...
	if (msg.from == this.playerID)
	{
		...
		if (cmpFormation) {
			let index = this.formationEntities.indexOf(msg.entity);
			if (index >=0)
				this.formationEntities.splice(index, 1);
		}
	}
	if (msg.to == this.playerID)
	{
		...
		if (cmpFormation)
			this.formationEntities.push(msg.entity);
	}
};
