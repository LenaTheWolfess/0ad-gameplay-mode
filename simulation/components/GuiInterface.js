...
GuiInterface.prototype.GetSimulationState = function()
{
	...
	for (let i = 0; i < numPlayers; ++i)
	{
		...
		ret.players.push({
			...
			"formationEntities": cmpPlayer.GetFormationEntities(),
...
GuiInterface.prototype.GetEntityState = function(player, ent)
{
	...
	let cmpEnergy = Engine.QueryInterface(ent, IID_Energy);
	if (cmpEnergy)
		ret.energy = {
			"points": cmpEnergy.GetPoints(),
			"maxPoints": cmpEnergy.GetMaxPoints(),
			"regenRate": cmpEnergy.GetRegenRate()
		};
	...
	if (cmpGarrisonHolder)
		ret.garrisonHolder = {
			"attackerEntities": cmpGarrisonHolder.GetAttackerEntities(),
	...
	if (cmpAttack)
	{
		...
		for (let type of types)
		{
			...
			ret.attack[type].allowed = cmpAttack.CanRange();
	...
	if (cmpResourceSupply)
		ret.resourceSupply = {
			...
			"workstation": cmpResourceSupply.IsWorkStation()
		};
...
GuiInterface.prototype.CanCharge = function(player, data)
{
	let ent = data.entity;
	let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
	if (!cmpUnitAI)
	...
	if (cmpUnitAI.IsFormationMember()) {
		ent = cmpUnitAI.GetFormationController();
	}
	let cmpFormation = Engine.QueryInterface(ent, IID_Formation);
	if (!cmpFormation) {
//		warn("no formation tries to charge");
		return false;
	}
	return cmpFormation && cmpFormation.CanCharge(data.target);
};
