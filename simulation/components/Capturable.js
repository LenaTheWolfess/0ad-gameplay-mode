...
Capturable.prototype.GetAttackRate = function()
{
	let cmpGarrisonHolder = Engine.QueryInterface(this.entity, IID_GarrisonHolder);
	if (!cmpGarrisonHolder || !cmpGarrisonHolder.IsAttackedInside())
		return 0;
	return cmpGarrisonHolder.GetAttackPower();
}

Capturable.prototype.GetMainAttacker = function()
{
	let cmpGarrisonHolder = Engine.QueryInterface(this.entity, IID_GarrisonHolder);
	if (!cmpGarrisonHolder)
		return 0;
	return cmpGarrisonHolder.GetMainAttacker();
}
...
Capturable.prototype.TimerTick = function()
{
	...
	let attackRate = this.GetAttackRate();
	let mainAttacker = this.GetMainAttacker();
	let takenCp = 0;
	if (attackRate && mainAttacker != INVALID_PLAYER) {
		takenCp = this.Reduce(attackRate, mainAttacker);
	}
	if (modifiedCp + takenCp != 0)
		return;
*/
	if (this.GetRegenRate() || this.GetAttackRate()) {
		let cmpGarrisonHolder = Engine.QueryInterface(this.entity, IID_GarrisonHolder);
		if (cmpGarrisonHolder)
			if (cmpGarrisonHolder.PerformInFigth())
				return;
	}
...
Capturable.prototype.CheckTimer = function()
{
	...
	let attack = cmpGarrisonHolder && cmpGarrisonHolder.IsAttackedInside() ? cmpGarrisonHolder.GetAttackPower() : 0;
	if (regenRate == 0 && decay == 0 && attack == 0)
		return;

	var cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
	this.timer = cmpTimer.SetInterval(this.entity, IID_Capturable, "TimerTick", 1000, 1000, null);
	Engine.PostMessage(this.entity, MT_CaptureRegenStateChanged, { "regenerating": true, "regenRate": regenRate, "territoryDecay": decay, "garrisonAttack": attack });
};
