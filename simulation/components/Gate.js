...
Gate.prototype.OperateGate = function()
{
	...
	if (this.opened && (this.allies.length == 0 || this.locked))
		this.CloseGate();
	else if (!this.opened && this.allies.length && !this.locked)
		this.OpenGate();
};

Gate.prototype.IsBroken = function()
{
	return this.broken;
};
...
Gate.prototype.IsOpened = function()
{
	return this.opened;
}

Gate.prototype.OnHealthChanged = function(msg)
{
	let cmpHealth = Engine.QueryInterface(this.entity, IID_Health);
	if (!cmpHealth)
		return;

	let percentage = cmpHealth.GetHitpoints() / cmpHealth.GetMaxHitpoints();

	if (percentage < 0.5 && !this.broken) {
		this.broken = true;
		this.locked = false;
	...
	else if (percentage > 0.8 && this.broken)
	{
		this.broken = false;
	}
}
...
Gate.prototype.CloseGate = function()
{
	...
	if (collisions.length && !this.locked)
	{
		if (!this.timer)
		{
