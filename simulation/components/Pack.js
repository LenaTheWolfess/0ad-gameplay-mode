...
Pack.prototype.PackProgress = function(data, lateness)
{
	...
	if (cmpAttack) {
		ammo = cmpAttack.GetAmmoLeft();
		rangeAttack  = cmpAttack.CanRange();
	}
	...
	if (newEntity){
		...
		let cmpAttackNew = Engine.QueryInterface(newEntity, IID_Attack);
		if (cmpAttackNew) {
			if (rangeAttack)
				cmpAttackNew.SetCanRange();
			else
				cmpAttackNew.SetNoRange();
			
			cmpAttackNew.SetAmmo(ammo);
		}
	}
