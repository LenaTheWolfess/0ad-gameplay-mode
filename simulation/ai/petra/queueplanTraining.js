...
m.TrainingPlan = function(gameState, type, metadata, number = 1, maxMerge = 5)
{
	...
	let batchSize = this.template.batchSize(trainer);
	this.cost = new API3.Resources(this.template.cost(trainer), +this.template._template.Cost.Population*batchSize);

	this.category = "unit";
	this.number = number*batchSize;
