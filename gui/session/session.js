...
function initFormationEntities() 
{
	Engine.GetGUIObjectByName("formationEntityPanel").children.forEach((button, slot) => {

		button.onPress = function() {
			let formationEnt = g_FormationEntities.find(ent => ent.slot !== undefined && ent.slot == slot);
			if (!formationEnt)
				return;
			...
			g_Selection.addList(formationEnt.members);
		};
		...
		button.onDoublePress = function() {
			...
			if (formationEnt) {
				g_Selection.reset();
				g_Selection.addList(formationEnt.members);
				moveTo(getEntityOrHolder(formationEnt.ent));
			}
		};
	});
}
...
function updateFormationEntities()
{
	let formationEnts =
		g_ViewedPlayer == -1 ?
			GetSimState().players.reduce((ents, pState) => ents.concat(pState.formationEntities), []) :
			GetSimState().players[g_ViewedPlayer].formationEntities;

	g_FormationEntities = g_FormationEntities.filter(formationEnt => formationEnts.find(ent => ent == formationEnt.ent));
...
function displayFormationEntities()
{
	let buttons = Engine.GetGUIObjectByName("formationEntityPanel").children;

	buttons.forEach((button, slot) => {

		if (button.hidden || g_FormationEntities.some(ent => ent.slot !== undefined && ent.slot == slot))
			return;

		button.hidden = true;
		stopColorFade("formationEntityHitOverlay[" + slot + "]");
	});

	for (let displayIndex = 0; displayIndex < Math.min(g_FormationEntities.length, buttons.length); ++displayIndex)
	{
		let formationEnt = g_FormationEntities[displayIndex];

		let slot = formationEnt.slot === undefined ?
