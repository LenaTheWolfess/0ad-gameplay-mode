...
var g_UnitActions =
{
	...
	"rotate-to-point":
	{
		...
		{
			if (!someUnitAI(selection) ||
			    !Engine.HotkeyIsPressed("session.rotate.toPoint") ||
			    !getActionInfo("rotate-to-point", target, selection).possible)
				return false;
	...
	"capture":
	{
		...
		"getActionInfo": function(entState, targetState)
		{
			if (!entState.attack || (!targetState.hitpoints && !targetState.capturePoints))
				return false;
	...
	"attack-charge":
	{
		...
		"getActionInfo": function(entState, targetState)
		{
			...
			return {
				"possible": Engine.GuiInterfaceCall("CanCharge", {
					"entity": entState.id,
					"target": targetState.id
				})
			};
	...
	"unload": {
		"getInfo": function(entStates)
		{
			if (entStates.every(entState => {
				...
				return !parent || !parent.garrisonHolder || (parent.garrisonHolder.entities.indexOf(entState.id) == -1 && parent.garrisonHolder.attackerEntities.indexOf(entState.id) == -1);
			}))
				return false;
...
function isUndeletable(entState)
{
	...
	if (entState.attackerEntities && entState.attackerEntities.length!=0)
		return translate("You cannot destroy this entity because there are attacking troops");
...
function getActionInfo(action, target, selection)
{
	...
	if (!target) // TODO move these non-target actions to an object like unit_actions.js
	{
		...
		return {
			"possible": ["move", "run", "attack-move", "remove-guard", "patrol", "rotate-to-point"].indexOf(action) != -1
		};
	}
