...
var g_FormatChatMessage = {
	...
	"trained": msg => formatTrainedMessage(msg),
	"broken-gate": msg => formatBrokenGateMessage(msg),
...
{
	"trained": function (notification, player)
	{
		...
		addChatMessage({
			...
			"unitCount": notification.unitCount,
			"rank": notification.rank
		});
	},
	"broken-gate": function (notification, player)
	{
		if (player != g_ViewedPlayer)
			return;
...
function colorizeByPlayer(text, playerID)
{
	let playerColor = playerID > -1 ? rgbToGuiColor(g_DisplayedPlayerColors[playerID]) : "white";
	return coloredText(text, playerColor);
}
...
function formatBrokenGateMessage(msg)
{
	if (msg.player != g_ViewedPlayer)
		return "";

	let message = "Your %(gate)s has been broken.";

	return sprintf(message, {"gate": colorizeByPlayer("gate", msg.player)});
}

function formatTrainedMessage(msg)
{
	if (msg.player != g_ViewedPlayer)
		return "";

	if (!msg.maxLevel) {
		let message = msg.unitCount == 1? "%(unit)s has been trained to rank %(rank)s." :
			 "%(count)s x %(unit)s have been trained to rank %(rank)s."
		if (msg.unitCount == 1)
			return sprintf(message, {"unit": colorizeByPlayer(msg.unitName, msg.player),"rank": colorizeByPlayer(msg.rank, msg.player)});
		
		return sprintf(message, {"count": msg.unitCount, "unit": colorizeByPlayer(msg.unitName, msg.player),"rank": colorizeByPlayer(msg.rank, msg.player)});
	}

	if (msg.unitCount && msg.unitCount > 1) {
		let message = "%(count)s %(unit)s achieved maximum training.";
		return sprintf(message, {"count": msg.unitCount,"unit": colorizeByPlayer(msg.unitName, msg.player)});
	}
	let message = translate("%(unit)s achieved maximum training.");
	return sprintf(message, {"unit": colorizeByPlayer(msg.unitName, msg.player)});
}
