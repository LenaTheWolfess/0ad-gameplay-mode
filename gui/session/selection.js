...
EntityGroups.prototype.add = function(ents)
{
	for (let ent of ents)
	{
		...
		if (entState.unitAI && entState.unitAI.formationController) {
			formationKey = entState.unitAI.formationController;
		} else {
			formationKey = key;
		}
		...
		if (entState.player !== undefined) {
			...
			formationKey = "p" + entState.player + "&" + formationKey;
		}
		...
		if (this.formGroup[formationKey])
			this.formGroup[formationKey] += 1;
		else
			this.formGroup[formationKey] = 1;
...
EntityGroups.prototype.removeEnt = function(ent)
{
	...
	let formKey = this.formEnts[ent];
	
	delete this.formEnts[ent];
	--this.formGroup[formKey];
	
	if (this.formGroup[formKey] == 0)
		delete this.formGroup[formKey];
};
...
EntityGroups.prototype.getFormKeys = function()
{
	return Object.keys(this.formGroup).sort();
}
...
EntityGroups.prototype.getEntsFormationGrouped = function()
{
	return this.getFormKeys().map(key => ({
		"ents": this.getEntsByFormKey(key),
		"key": key
	}));
}
