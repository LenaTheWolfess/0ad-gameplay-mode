...
function getFormationTooltip(template)
{
	if(!template.isFormation)
		return "";
	
	return	[
				"minimal members: " + template.requiredMemberCount + "\n" +
				"max rows: " + template.rows.max + "\n" +
				"min rows: " + template.rows.min + "\n" +
				"max columns: " + template.columns.max + "\n" +
				"min columns: " + template.columns.min + "\n" +
				"widthDepthRatio: " + template.widthDepthRatio + "\n" +
				"unit separation width: " + template.unitSeparation.width + "\n" +
				"unit separation depth: " + template.unitSeparation.depth + "\n" 
			];
}
...
function getSpeedTooltip(template)
{
	...
	let walk = template.speed.walk ? template.speed.walk.toFixed(1) : 0;
	let run = template.speed.run ? template.speed.run.toFixed(1) : 0;
