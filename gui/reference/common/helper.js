...
{	
	if (!template.requiredTechnologies.length)
		return g_ParsedData.phaseList[0];

	for (let tech of template.requiredTechnologies) { 
		if (basename(tech).startsWith("phase"))
			return getActualPhase(tech);
	}
	return getPhaseOfTechnology(template.requiredTechnologies[0]);
}
