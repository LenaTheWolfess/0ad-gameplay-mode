...
function formationDataExists(templateName)
{
	return Engine.FileExists("simulation/templates/" + templateName + ".xml");
}
/**
...
function loadFormationTempalte(templateName)
{
	...
	if (!template.Formation)
		return loadEntityTemplate(templateName);
	let parsed = GetTemplateDataHelper(template, null, g_AuraData, g_ResourceData, g_DamageTypes, g_CurrentModifiers);
	parsed.isFormation = true;
	parsed.name.internal = templateName;
	parsed.speed = null;
	parsed.requiredMemberCount = template.Formation.RequiredMemberCount || 0;
	parsed.description = template.Formation.Tooltip;
	parsed.rows = {};
	parsed.columns = {};
	parsed.unitSeparation = {};
	parsed.rows.max = template.Formation.MaxRows || "infinity";
	parsed.rows.min = template.Formation.MinRows || 1;
	parsed.columns.max = template.Formation.MaxColumns || "infinity";
	parsed.columns.min = template.Formation.MinColumns || 1;
	parsed.widthDepthRatio = template.Formation.WidthDepthRatio || 1;
	parsed.unitSeparation.width = template.Formation.UnitSeparatonWidthMultiplier || 1;
	parsed.unitSeparation.depth = template.Formation.UnitSeparatonDepthMultiplier || 1;
	return parsed;
}
